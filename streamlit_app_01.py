...
def main():
    ...
    with main_col:
        # 主要内容区域 - 使用选项卡
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["实时监控", "数据分析", "测试验证", "交互优化", "系统信息"])
...
def show_interactive_optimization():
    ...
    <strong>完整优化流程：</strong>环境数据采集 → 注意力机制策略选择 → 时频联合滤波 → 深度残差网络增强 → 质量评估与参数调整
    </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with display_col:
        ...
        with st.expander("🔍 查看完整分析图表", expanded=False):
            ...
            with tab3:
                ...
                fig_att.update_yaxes(title_text="权重值", gridcolor='rgba(255,255,255,0.1)', row=1, col=1)
